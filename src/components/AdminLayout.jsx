...
const adminNavLinks = [
  { to: '/console/admin', label: 'Dashboard', icon: (
  ...
  { to: '/console/admin/companies', label: 'Companies', icon: (
  ...
  { to: '/console/admin/gallery', label: 'Gallery', icon: (
...
export default function AdminLayout() {
  ...
  const handleLogout = () => {
    logout();
    navigate('/console/admin/login');
  };

  return (
    <div className="min-h-screen flex bg-surface-50">
      {/* Sidebar */}
      <aside className="w-64 flex-shrink-0 bg-surface-900 flex flex-col">
        {/* Brand */}
        <div className="p-5 border-b border-surface-800">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-brand-600 rounded-xl flex items-center justify-center font-bold text-white text-sm font-display">
              JN
            </div>
            <div>
              <p className="font-semibold text-white text-sm">Admin Panel</p>
              <p className="text-surface-500 text-xs truncate max-w-[140px]">{admin?.email}</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          <p className="text-[10px] uppercase tracking-wider text-surface-600 font-semibold px-4 pt-3 pb-2">Menu</p>
          {adminNavLinks.map(({ to, label, icon, end }) => (
            <NavLink
              key={to}
              to={to}
              end={end}
